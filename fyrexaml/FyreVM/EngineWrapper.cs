...
namespace Zifmia.FyreVM.Service
{
    public class EngineWrapper
    {
        ...
        private void vm_OutputReady(object sender, OutputReadyEventArgs e)
        {
            ...
            if ((wrapperState == VMWrapperState.LoadGame && (requestType == VMRequestType.StartGame || requestType == VMRequestType.NoCommand)) || wrapperState == VMWrapperState.RunCommand)
            {
            ...
            if (wrapperState == VMWrapperState.RequestSave)
            {
                if (vm.LastSave != null)
                {
                    outSaveFile = vm.LastSave.ToArray();
                    saveStream = vm.LastSave;
                }
                vm.Stop();
            }

            if (wrapperState == VMWrapperState.RunCommand || (wrapperState == VMWrapperState.LoadGame && (requestType == VMRequestType.StartGame || requestType == VMRequestType.NoCommand)))
            {
